...
def inference(val_loader, net, arch, loss_fn, epoch, calc_metrics=False):
    """
    ...
    for val_idx, data in enumerate(val_loader):
        ...
        img = assets['predictions'][0]

        # countlabel = []
        # for i in range(img.shape[0]):
        #     for j in range(img.shape[1]):
        #         if img[i][j] not in countlabel:
        #             countlabel.append(img[i][j])
        # print(countlabel)

        # print(img[0][0].__class__)
        # for i in range(img.shape[0]):
        #     for j in range(img.shape[1]):
        #         img[i][j] = labels[img[i][j]].color


        im=Image.fromarray(np.uint8(img))

        im.save("/root/final-term/InverseForm/result/{}.png".format(img_names))
...
def main(output_dir, model_path, has_edge=False, model_summary=False, arch='ocrnet.AuxHRNet', 
         hrnet_base='18', num_workers=1, split='val', batch_size=1, crop_size='1024,2048', 
